...
from .runtime import RuntimeMode, detect_runtime, get_dbutils, get_runtime_parameter
...
class TrainingConfig:
    ...
    deployment_manifest_path: Path = Path(".local/deployment/local_deployment_manifest.json")
...
class DeploymentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.smoke_test_rows < 1:
            raise ValueError("smoke_test_rows debe ser positivo.")
        if self.serving_timeout_seconds < 1:
            raise ValueError("serving_timeout_seconds debe ser positivo.")
        if self.serving_poll_seconds < 0:
            raise ValueError("serving_poll_seconds no puede ser negativo.")
...
def build_config(
    *,
    ...
    return TrainingConfig(
        ...
        artifact_location=_runtime_artifact_location(
            setting("IRIS_ARTIFACT_LOCATION", str(runtime_config.get("artifact_location", ""))),
            runtime_mode=runtime_mode,
        ...
        deployment_manifest_path=_resolve_project_path(
            str(
                runtime_config.get(
                    ...
                    ".local/deployment/local_deployment_manifest.json",
                )
            ),
            config_root,
        ),
...
def _runtime_artifact_location(value: str, *, runtime_mode: RuntimeMode, project_root: Path) -> str:
    """Canonicalize local artifact paths relative to the repository root."""

    if runtime_mode != "local" or not value:
        return value
    return str(_resolve_project_path(value, project_root))
