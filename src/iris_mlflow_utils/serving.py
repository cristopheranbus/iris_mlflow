...
def _build_payload(columns: list[str], rows: list[list[Any]]) -> dict[str, Any]:
    """Validate tabular input and build the Databricks dataframe_split payload."""
    ...
    return {"dataframe_split": {"columns": columns, "data": rows}}


def _raise_for_status(response: requests.Response) -> None:
    """Translate Databricks HTTP failures into stable, credential-safe errors."""
...
def predict(columns: list[str], rows: list[list[Any]]) -> list[Any]:
    ...
    payload = _build_payload(columns, rows)
    try:
